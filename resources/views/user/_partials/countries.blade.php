...
<input type="hidden" name="country" id="selected-country" value="{{ old('country', $profileData->country) }}"> <!-- For the country name -->
<input type="hidden" name="country_code" id="selected-country-code" value="{{ old('country_code', $profileData->country_code) }}"> <!-- For the country code -->

<div class="custom-select-africa mb-3 col-lg-6">
    <label for="Countries">Country</label>
    <div class="selected-item-africa">
        <div class="selected-item-africa">
            <img src="" alt="Selected Flag" id="selected-flag-africa"> <!-- Image for selected flag -->
                        <!-- Hidden input to store selected country -->

            <input type="text" placeholder="Search countries..." id="search-input-africa"
                class="form-control country-form ">
        </div>
    </div>
...
<script>
    ...
    // Select an option and show both flag and name
    selectItemsAfrica.addEventListener('click', (e) => {
        ...
        if (target) {
            ...
            // Set the input value to the selected country name
            selectedItemAfrica.value = countryName;

            // Set the selected flag image in the input field
            selectedFlagAfrica.src = imgSrc;
            selectedFlagAfrica.style.display = 'inline'; // Make the flag visible
   ...
   function selectCountry(countryCode, countryName) {
    ...
    // Update the hidden input with the selected country name and code
    document.getElementById('selected-country').value = countryName; // Store country name
    document.getElementById('selected-country-code').value = countryCode; // Store country code

    // Update the input field with the selected country's name (optional for display purposes)
